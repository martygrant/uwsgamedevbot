function guildMemberAdd(member)
{
    ...
    const welcomeString = `Welcome ${member.toString()} to the UWS Game Dev Society!\n\nPlease check out ${rulesChannel.toString()} and set your server nickname to your real name.\nVisit ${announcementChannel.toString()} to see what events are coming up! Why not also ${introductionChannel.toString()}?\n\nPlease conduct yourself professionally in public-facing channels like ${lobbyChannel.toString()}. Thanks!`;

    lobbyChannel.send(welcomeString).catch(console.error);
    member.send(welcomeString).catch(console.error);
}
